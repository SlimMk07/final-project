...
let places = [
  {name:"ariana",     position:{lat:36.866111, long:10.168639}, exists:false},
  {name:"béja",       position:{lat:36.729500, long:9.190950}, exists:false},
  {name:"ben-arous",  position:{lat:36.749851, long:10.235764}, exists:false},
  {name:"bizerte",    position:{lat:37.283385, long:9.860990}, exists:false},
  {name:"gabès",      position:{lat:33.889633, long:10.101371}, exists:false},
  {name:"gafsa",      position:{lat:34.430137, long:8.779670}, exists:false},
  {name:"jendouba",   position:{lat:36.503216, long:8.777432}, exists:false},
  {name:"kairouan",   position:{lat:35.677433, long:10.099470}, exists:false},
  {name:"kasserine",  position:{lat:35.170811, long:8.835412}, exists:false},
  {name:"kébili",     position:{lat:33.704952, long:8.971195}, exists:false},
  {name:"le-kef",     position:{lat:36.171562, long:8.704727}, exists:false},
  {name:"mahdia",     position:{lat:35.505289, long:11.047079}, exists:false},
  {name:"la-manouba", position:{lat:36.808438, long:10.083754}, exists:false},
  {name:"médenine",   position:{lat:33.345114, long:10.493660}, exists:false},
  {name:"monastir",   position:{lat:35.762897, long:10.817483}, exists:false},
  {name:"nabeul",     position:{lat:36.451692, long:10.722895}, exists:false},
  {name:"sfax",       position:{lat:34.740753, long:10.749559}, exists:false},
  {name:"sidi-bouzid",position:{lat:35.037369, long:9.485793}, exists:false},
  {name:"siliana",    position:{lat:36.079524, long:9.297831}, exists:false},
  {name:"sousse",     position:{lat:35.828737, long:10.632647}, exists:false},
  {name:"tataouine",  position:{lat:32.926300, long:10.448858}, exists:false},
  {name:"tozeur",     position:{lat:33.925508, long:8.119843}, exists:false},
  {name:"tunis",      position:{lat:36.797840, long:10.178993}, exists:false},
  {name:"zaghouan",   position:{lat:36.407560, long:10.141831}, exists:false}
]
...
function myMap() {
    ...
    let arr = []
    for(let p of places){
        let x = speakers.filter((elt)=>elt.place.toLocaleLowerCase()==p.name)
        if (x.length>0) {
            p.exists=true
            arr.push(x)
        }
            ...
            `<section class="directory-map-marker degree-">
                <div class="content-left">
                    <div class="avatar-container">
                        <img class="avatar" alt="${f.name.first+" "+f.name.last}" src="${f.picture}">
                    </div>
                    <div class="ng-isolate-scope" track-category="Directory - infowindow" social="item.data">
                        <ul class="shares horizontal-list">
                            <li class="ng-scope" data-ng-if="social.linkedin">
                                <a class="dir-share dir-share-linkedin image-for-text ng-binding" href="https://www.linkedin.com/in/alixjklingenberg" target="_blank" rel="noopener" data-track-label="${f.name.first+" "+f.name.last}" data-track-action="Click: share linkedIn" data-track-category="Directory - infowindow" data-ng-href="https://www.linkedin.com/in/alixjklingenberg">${places[index]['name']}</a>
                            </li>
                            <li class="ng-scope" data-ng-if="social.twitter">
                                <a class="dir-share dir-share-twitter image-for-text ng-binding" href="https://twitter.com/evolutionised" target="_blank" rel="noopener" data-track-label="${f.name.first+" "+f.name.last}" data-track-action="Click: share Twitter" data-track-category="Directory - infowindow" data-track="" data-ng-href="https://twitter.com/evolutionised">Share ${f.name.first+" "+f.name.last}'s profile on Twitter</a>
                            </li>
                        </ul>
                    </div>
                </div>
                <div class="content-right-top">
                    <h3 class="ng-binding" data-ng-bind="::item.data.title || item.data.name">${f.name.first+" "+f.name.last}</h3>
                    <h4 class="ng-binding">${f.place}</h4>
                </div>
                <div class="content-right-bottom degree-">${f.background}</div>
            </section>`
          }
          str+= "</div>"
...
function filter(){
  ...
  let filteredSpreakers=speakers
  ...
  if(filterGender.length>0)     filteredSpreakers=filteredSpreakers.filter((elt)=> filterGender.includes(elt.gender))
